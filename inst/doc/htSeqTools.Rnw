...
Notice that any sequence repeating over 40 times
...
pool <- RangedDataList(ip=ctrl, ctrl=ctrl)
...
<<enrichedRegions>>=
regions <- enrichedRegions(sample1=ip,sample2=ctrl,minReads=10,p.adjust.method='BY',pvalFilter=.05,twoTailed=FALSE)
head(regions)
nrow(regions)
@ 
...
The result is stored in an object of class \texttt{gridCover},
which contains the coverage in the slot \texttt{cover}
and some summary information about each region in \texttt{viewsInfo}.
...
ylim <- c(0,max(gridcov@viewsInfo[['maxCov']]))
plot(gridcov, ylim=ylim,lwd=2)
for (i in 1:nrow(regions)) lines(gridcov@cover[i,], col='gray', lty=2)
@ 
