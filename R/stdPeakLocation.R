...
function(x, startpos='start_position', endpos='end_position', strand='strand', distance, main='', xlab='Distance relative to feature length', xaxt='n', xlim=c(0,1), densityType='kernel', nbreaks=10, ...) {
...
stdPeakLocationBase <- function(st, end, startpos, endpos, strand, distance, main='', xlab='Distance relative to feature length', xaxt='n', xlim=c(0,1), densityType='kernel', nbreaks=10, ...) {
  if (missing(distance)) {
    peakloc <- .5*(st+end)
    st <- startpos; end <- endpos
...
PeakLocationBase <- function(st, end, startpos, endpos, strand, distance, peakDistance=1000, main='', xlab='Distance (bp)',densityType='Kernel',breaks,...) {
  if (missing(distance)) {
    peakpos <- .5*(st+end)
    if ((is.character(strand) | is.factor(strand))) {
      distance <- ifelse(strand=='+',  peakpos - startpos, endpos - peakpos)
    } else if (is.numeric(strand)) {
      distance <- ifelse(strand>= 0, peakpos - startpos, endpos - peakpos)
    } else {
      stop('Invalid strand information')
    }
  }
  distance <- distance[!is.na(distance)]
  distance <- distance[(distance> -2*peakDistance) & (distance<3*peakDistance)]
  if (densityType=='kernel') {
